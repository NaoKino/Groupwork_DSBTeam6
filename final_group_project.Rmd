...
author: "Thaisiri, Get, Naoya, Ronapat"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
#summarise how many frauds each year
card_fraud %>%
  filter(is_fraud == 1) %>% 
  group_by(year) %>%
  summarise(count = n())

# summarizes the total amount of legitimate and fraudulent transactions per year and calculate the % of fraudulent transactions, in US$ terms.
card_fraud %>% 
  ...
  mutate (percent_fraud = 100*(sum_amt/sum(sum_amt,year = year)))
...
-   `job`, Job of Credit Card Holder
    ...
    card_fraud <- card_fraud %>% 
      mutate(
        #strings to factor - category and job
...
-   `state`, State of Credit Card Holder
    ...
    library(patchwork)
        ...
        city_geo_address <- card_fraud %>% 
          filter(is_fraud == 1) %>% 
          mutate(city_geo = geo(city, method = "osm"))
        
        city_geo_for_graph <- city_geo_address %>% 
          count(city, sort=TRUE) %>%
          mutate(address_geo = purrr::map(city, geo, method = "osm")) %>%
        ...
        geom_point(data = city_geo_for_graph, # then we add points
          aes(x = long, y = lat),
          size = 2,
          colour = "#001e62") +
        theme_void()
...
# Count and calculate proportions for the training set
card_fraud_train_summary <- card_fraud_train %>%
  group_by(is_fraud) %>%
  summarise(n = n()) %>%
  mutate(percentage = n / sum(n) * 100)

print(card_fraud_train_summary)

# Count and calculate proportions for the test set
card_fraud_test_summary <- card_fraud_test %>%
  group_by(is_fraud) %>%
  summarise(n = n()) %>%
  mutate(percentage = n / sum(n) * 100)

print(card_fraud_test_summary)

#After checking, percentages are similar.
```
...
```{r, define_recipe}
#In order to address the heavy left skewness and the presence of a few very large transactions in the data, a log transformation will be applied to the "amt" variable. This transformation will be included as a step in the recipe.
...
```{r}
#In highly imbalanced datasets, accuracy alone can be misleading due to the disproportionate representation of the majority class. Therefore, it is important to consider additional metrics that provide a more comprehensive evaluation of the model's performance.

#Given the imbalanced nature of the data, the following metrics are typically recommended:

#Sensitivity/Recall/True Positive Rate: This metric measures the ability of the model to correctly identify positive instances (fraudulent transactions) from the actual positive cases. It is calculated as the ratio of true positives to the sum of true positives and false negatives.

#Specificity/True Negative Rate: This metric measures the ability of the model to correctly identify negative instances (non-fraudulent transactions) from the actual negative cases. It is calculated as the ratio of true negatives to the sum of true negatives and false positives.

#Area Under the Receiver Operating Characteristic Curve (AUC-ROC): This metric provides an aggregate measure of the model's ability to distinguish between positive and negative instances across various probability thresholds. A higher AUC-ROC indicates a better discriminatory power of the model.
```
